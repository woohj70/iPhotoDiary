...
-(EXFTag*) tagDefinition: (NSNumber*) aTagId{
	Debug(@"tagDefinition.aTagId = %x", aTagId);
	NSArray *keys = [self.tagDefinitions.definitions allKeys];
	
	for (NSInteger i = 0; i < [keys count]; i++) {
		Debug(@"keys.aTagId = %@", [keys objectAtIndex:i]);
	}
	
	Debug(@"value = %@", [self.tagDefinitions.definitions objectForKey:aTagId]);
...
-(id) tagValue: (NSNumber*) aTagId{
   ...
   Debug(@"value  = %@", aTagId); 
	...
	NSArray *keys = [self.keyedTagValues allValues];
